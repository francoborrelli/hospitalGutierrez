...
const getData = (patient, user) => {
  const data = [
    ...
    {
      title: 'Género',
      data: patient.gender === 'male' ? 'Masculino' : 'Femenino'
    },
    {
      title: 'Teléfono',
      data: patient.phone || '-'
  ...
  const demographicData = user.permissions.includes('datosDemograficos_show')
    ? [
        {
        ...
        {
          title: '¿Heladera?',
          data: patient.hasRefrigerator ? 'Si' : 'No'
        },
        {
          title: '¿Mascotas?',
          data: patient.hasPet ? 'Si' : 'No'
        },
        {
          title: '¿Electricidad?',
          data: patient.hasElectricity ? 'Si' : 'No'
        }
      ]
    : [];
...
const patientCard = props => {
  ...
  const elements = getData(props.patient, props.user).map((element, index) => (
    <Element
      key={index}
      sm={element.sm || 12}
      title={element.title}
